SELECT ticket.id AS "Nº Ticket", company.name as Cliente, user.name AS "Técnico Responsable", grupo.name as "Tipo Requerimiento(Grupo)", 
  ...
  ticket.date_send_to_closedtechnical as "Fecha de envío a cierre", round(sla2(ticket.created_at, ticket.date_send_to_closedtechnical) / 60) AS "Tiempo envio a cierre"
...
WHERE ticket.created_at BETWEEN '2017-03-01' AND '2017-09-30'
ORDER BY ticket.id;